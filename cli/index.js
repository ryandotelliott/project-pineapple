		...
		if (!fs.existsSync(path)) {
			...
			try {
				...
				try {
					...
					console.log(
						c.red("Unable to open SQLite database. Please restart and try again.")
					);
					process.exit(1);
				}
				do {
			...
			try {
				...
				console.log(
					c.red("Error: Unable to authenticate. Please check config.json and try again.")
				);
				console.error(err);
				process.exit(1);
			}
	...
	return new Promise(async (resolve, reject) => {
		...
		if (menuAnswer.menuSelection == "Export Followers to .CSV") {
			...
			if (inquirerRes.confirm) {
				...
				console.log(c.red("config.json & followers.db successfully deleted.\n"));
...
async function DMFollowers() {
	...
	if (sortOption.sortingOption == "All Followers") {
		...
		if (firstColumn.list != "None / Continue") {
			...
			while (true) {
				...
				availableColumns.splice(
					availableColumns.findIndex((item) => item.name == additionalColumns.list),
					1
				);
				rankedColumns.push(firstColumn.list);
			}
			...
			SQLColumnsString = SQLColumnsString.substring(0, SQLColumnsString.length - 2);
	...
	console.log("\n");
	console.log(c.magenta("Message Preview: ") + messagePreview);
	console.log("\n");
